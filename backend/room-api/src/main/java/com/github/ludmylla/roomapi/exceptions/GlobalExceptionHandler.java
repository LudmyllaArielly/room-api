...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> resourceNotFoundException(ResourceNotFoundException ex, WebRequest webRequest) {
        ErroDetails erroDetails = new ErroDetails(new Date(), ex.getMessage(), webRequest.getDescription(false));
        return new ResponseEntity<>(erroDetails, HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<?> globalExceptionHandler(ResourceNotFoundException ex, WebRequest webRequest) {
        ErroDetails erroDetails = new ErroDetails(new Date(), ex.getMessage(), webRequest.getDescription(false));
